...
```text
⚠ リンク 1 本で一時ルームを作る      ⚠ 発行する側だけが Google で名乗る(adr/0030)
⚠ 入れるかは Host がその場で決める    ⚠ 招待された側はアカウントを作らない(adr/0017)
閉じると会話の内容は残らない          Cloudflare の 1 オブジェクトで動き、DB を持たない
```

⚠ **合言葉は無い**([`adr/0017`](adr/0017-let-the-host-decide-who-comes-in-instead-of-a-passphrase.md))。
⚠ **1 プロセスでもない**([`adr/0022`](adr/0022-start-on-cloudflares-free-tier-with-one-durable-object-per-room.md))。
⚠ **この節が書かれた 2026-09-05 から、⚠ 入口も土台も動いている。**

### ⚠⚠ 差別化を「機能が珍しいか」で書かない

⚠ **2026-09-08 に この節を組み替えた。** ⚠ **前の版は機能で並べていて、⚠ ほとんどの行が
「珍しくない」に落ちていた** — ⚠ **機能で比べるかぎり、⚠ 個々の機能はどれも既にある。**

⚠ **そして前の版が「珍しい」と書いていた行のうち 1 つは、⚠ もう存在しない:**
⚠ **「URL と合言葉を別経路で渡す入口」は
[`adr/0017`](adr/0017-let-the-host-decide-who-comes-in-instead-of-a-passphrase.md)
で無くなった。** ⚠ **差別化として名指ししていたものが消えたのに、⚠ 表は更新されていなかった。**

⚠ **so 軸を 2 本にする: ⚠ どんな関係を作るか、⚠ そして何を残さないか。**

| ⚠ どんな関係を作るか | ⚠ 何を残さないか |
|---|---|
| ⚠ **発行する側は名乗る**(Google、[`adr/0030`](adr/0030-let-the-host-sign-in-with-google-and-keep-the-guest-anonymous.md)) | ⚠ **その人が何をしたかの記録は持たない** — ⚠ どのルームも、⚠ 誰が入ったかも、⚠ いつかも |
| ⚠ **招待された側は名乗らない** | ⚠ **継続的に識別する手段を持たない**(`PRODUCT.md` § 5) |
| ⚠ その場で部屋を出す | ⚠ **部屋そのものが残らない**([`adr/0010`](adr/0010-a-room-lives-while-somebody-is-in-it-and-not-longer.md)) |
| ⚠ **入れるかは その場で人が決める**([`adr/0017`](adr/0017-let-the-host-decide-who-comes-in-instead-of-a-passphrase.md)) | ⚠ **決めたことの記録を残さない** |
| ⚠ 終われば 関係も終わる | ⚠ 連絡先も履歴も残らない |
...
> ⚠ **そして関係は 部屋と一緒に終わる。**
>
> ⚠⚠ **この 組み合わせ が、⚠ 個々の機能ではなく 価値になる。**

⚠ **Owner 判断 2026-09-08。** ⚠ **H1〜H5 の上に乗る枠であり、⚠ H1 と H3 は D の実例に見える。**

⚠⚠ **この形は 狙って作られたのではない。** ⚠ **`adr/0030` の文脈は「Basic 認証では配って回る
鍵になる」であり、⚠ 運用の修理だった。** ⚠ **価値として置き直したのは そのあとである** —
⚠ **後から意味づけたことを、⚠ 設計したことのように書かない**
([`../.claude/rules/evidence.md`](../.claude/rules/evidence.md))。

⚠ **Meet や Zoom と機能単位で比べない。** ⚠ **他所の測定を持っておらず、⚠ 持ってきても
kagima についての測定ではない。** ⚠ **比べうるのは「関係が続くか / 終わるか」の 1 点だけで、
⚠ それも仮説である。**

| ⚠ 反証されるとしたら | ⚠ どこで聞くか |
|---|---|
| ⚠ **「消える」が 価値ではなく 不便 と言われる。** ⚠ 「あとで見返したい」が先に出る | ⚠ **E2**([kagima#41](https://github.com/hidetzu/kagima/issues/41)) |
| ⚠ **人は「一時的な関係」を作りたいのではなく、⚠ 関係を作らずに用を済ませたいだけ** — ⚠ その場合 売り方が変わる | ⚠ **E3**([kagima#42](https://github.com/hidetzu/kagima/issues/42)) |
| ⚠⚠ **招待された側が、⚠ 相手が名乗っているかを気にしない** — ⚠ 非対称が価値でなく 実装都合になる | ⚠ **E4**([kagima#43](https://github.com/hidetzu/kagima/issues/43))、⚠ **E1** の待っているあいだ |
| ⚠ **発行する側が「名乗るくらいなら使わない」と言う** | ⚠ **E3** |
| ⚠⚠ **「量産されない」を、⚠ 誰も 利用者の価値だと思わない** | ⚠⚠ **これが一番効く。** ⚠ **その場合 D は差別化仮説ではなく、⚠ 運営側の制約に格下げされ、⚠ 売り文句にはできない** |

⚠⚠ **最後の行を先に書いておく。** ⚠ **「量産を止める」は 我々の必要であって、⚠ 利用者の価値とは
限らない。** ⚠ **混ぜると、⚠ 運用の都合を製品の売りとして語ることになる。**
...
### ⚠⚠ 部屋の中で何が起きるか — ⚠ この節にいちばん大きく空いていた穴

⚠ **2026-09-08 に足した。** ⚠ **H1〜H5 は「誰が / いつ」を言っていて、⚠ 「中で何をするか」を
一つも言っていない。**

```text
⚠ 入口   ⚠ よく考えられている   adr/0017(誰が決めるか)、⚠ adr/0030(誰が名乗るか)、⚠ E1
⚠ 出口   ⚠ よく考えられている   何も残さない ― ⚠ PRODUCT.md § 5 の中心
⚠⚠ 中   ⚠⚠ ほぼ何も無い       「2 人がカメラとマイクで会話する」(PRODUCT.md § 3)、⚠ それだけ
```

### ⚠ コンセプトと実装が 違うことを言っている

> [`PRODUCT.md`](PRODUCT.md) § 1 — ⚠ **「招待した人にだけ、⚠ ひととき開く プライベート空間」**

⚠⚠ **「空間」と書いてあるのに、⚠ 中身は「通話」である。**

⚠ **そして入口は非対称である**(⚠ Host が招き、⚠ Host が決め、⚠ Host だけ名乗る)。
⚠ **なのに部屋の中は対称である** — ⚠ **両方がカメラを出し、⚠ 対等に話す。**

⚠ **「私の空間に、⚠ あなたを入れる」で始まって、⚠ 中に入ると「対等な通話」になる。**
⚠ **この不一致は 決着していない。** ⚠ **どちらが正しいかは ここでは決めない。**

### ⚠ 「相手に何をさせたいのか」の候補

| # | ⚠ 中で起きること | ⚠ いま可能か | ⚠ どの仮説に効くか |
|---|---|---|---|
| **1** | ⚠ **話す**(⚠ 対称) | ⚠ **いまこれ** | H1 H2 H3 |
| **2** | ⚠ **相手に見せる**(Host → Guest) | ⚠ **実装ゼロ。** ⚠ カメラを向けるだけ | H4 H5 |
| **3** | ⚠⚠ **相手に見せてもらう**(Guest → Host) | ⚠ **実装ゼロ** | ⚠⚠ **どの仮説にも無い** |
| **4** | ⚠ 渡す(⚠ 一時ファイル) | ⚠ 実装が要る | H4 |
| **5** | ⚠ 操作する(⚠ 遠隔) | ⚠ **遠い**(`PRODUCT.md` § 2 の将来方向) | H5 |

⚠⚠ **3 が どの仮説にも入っていない。**
⚠ **H4 は「一度だけ何かを 見せたい / 渡したい人」であり、⚠ 見せて もらう 側の話が無い。**

⚠ **3 は いまの入口の形と 向きが揃っている**: ⚠ **「入れる」と決めるのは Host であり、
⚠ 「あなたのそれを、⚠ 一度だけ見せてください」は その非対称と噛み合う** — ⚠ 修理の見積もり、
⚠ 症状、⚠ 現物、⚠ 部屋の様子。
⚠ **そして 2 より 3 のほうが「残らない」が効く** — ⚠ **見せる側が「残ってほしくない」と思う
からである。**

⚠⚠ **仮説として立てるかどうかは Owner 判断である**([`PRODUCT.md`](PRODUCT.md) § 6)。
⚠ **ここでは 穴が在ることだけを書く。** ⚠ **AI が仮説を 1 本増やすことはしない。**

### ⚠⚠ 2026-09-09 — ⚠ Owner が 6 つ目を選んだ

⚠ **上の 5 つのどれでもなかった。**

> ⚠ **6. ⚠ 共に見る(対称)** — ⚠ **画面共有 と ポインタ。**
> ⚠ **「誰かと 5〜20 分だけ、⚠ 同じものを見て『ここ』と言える」**
> ([`PRODUCT.md`](PRODUCT.md) § 1、⚠ Owner 決定)

⚠ **2(見せる)と 3(見せてもらう)は 片方向だった。** ⚠ **6 は 双方向である** — ⚠ **Host も
Guest も 「ここ」と指せる。**

⚠⚠ **これで「空間 対 通話」の不一致にも 答えが出た** — ⚠ **どちらでもない。**
⚠ **入口は非対称(⚠ Host が招き、⚠ Host だけ名乗る)、⚠ 中は対称(⚠ 共に見る)である。**
⚠ **不一致ではなく、⚠ そういう形だと決まった。**

⚠ **v0.1.0 に入るのは 画面共有・ポインタ・mute・camera on/off まで**
([`PRODUCT.md`](PRODUCT.md) § 3)。⚠ **ホワイトボードも 遠隔操作も 入らない。**

⚠⚠ **so この軸の問いは「何をさせたいか」から「⚠ それが実際に使われるか」に移った。**
⚠ **測り方は [`adr/0032`](adr/0032-count-what-was-used-never-what-was-seen.md)** —
⚠ **画面共有を始めたルームの割合、⚠ ポインタを使ったルームの割合、⚠ そのファネル。**
⚠ **そして「なぜ」は 数では分からない** — ⚠ **E2 / E3 / E4 が要る。**

### ⚠ この軸を反証するもの

| ⚠ 反証されるとしたら | ⚠ どこで |
|---|---|
| ⚠ **2 / 3 に対して「写真を送れば済む」と言われる** | ⚠⚠ **リアルタイムである必要が無い** — ⚠ **E4**([kagima#43](https://github.com/hidetzu/kagima/issues/43)) |
| ⚠ **3 に対して、⚠ 見せる側が「残ってほしくない」と思っていない** | ⚠ **むしろ証拠として残したい** — ⚠ **E4 の 2 問目がそのまま効く** |
| ⚠ **「空間」という言い方が伝わらず、⚠ 人が「通話」と思う** | ⚠ **E3**([kagima#42](https://github.com/hidetzu/kagima/issues/42))の C / D / E |

### ⚠ この軸が言えないこと

- ⚠ **どれが正しいかは 決めない。** ⚠ **5 つは 排他でもない。**
- ⚠ **「空間 対 通話」の不一致を、⚠ どちらに寄せるかも 決めない。** ⚠ **`PRODUCT.md` § 1 と § 3 に
  触る判断であり、⚠ Owner のものである。**
- ⚠ **2 と 3 は 実装ゼロで試せる** — ⚠ **が、⚠ まだ一度も試していない。**
  ⚠ **[E5](https://github.com/hidetzu/kagima/issues/44) が唯一この軸に触れる実験で、⚠ いまは
  5(自宅デバイス)寄りに書かれている。**
...
|---|---|---|---|---|
| **E1** | [hidetzu/kagima#40](https://github.com/hidetzu/kagima/issues/40) | ⚠ **非技術者 3 人に URL を渡して入ってもらい、⚠ 詰まった場所を記録する**(⚠ **合言葉は無い**)。⚠ **待っているあいだ その人が何をしたかも見る** | ⚠ **入口そのものが成立するか。** ⚠ **成立しないなら、⚠ どの方向へ広げても土台が無い** | ⚠ **ゼロ。** ⚠ 診断パネルの `waited alone` がそのまま手渡しの摩擦である(⚠ 実測 2026-09-08: **71.6s / 82.0s**、⚠ ただし Owner 自身の手渡し) |
...
### ⚠⚠ 推論ではなく、⚠ 実際に踏んで測った(2026-09-05)

⚠ **上は Cloudflare の文書から読んだことである。** ⚠ **so 自分で踏んだ。**
⚠ **`wrangler dev --local`。** ⚠ **ルームを作って放置し、⚠ 合言葉を入れ直すだけである。**
...
| 8 秒 | ⚠ **通った** |
| ⚠ **15 秒** | ⚠⚠ **通らない** |
| 25 / 30 / 90 / 150 / 210 / 270 秒 | ⚠ **通らない** |

⚠ **境界は 8 秒と 15 秒のあいだにある。** ⚠ **文書の「10 秒でアイドル hibernate」と噛み合う。**

⚠ **同時に `storage` を問い合わせた: ⚠ 0 キー。**
⚠ **so 消えた理由は「書いたものが消えた」ではなく、⚠ 「書いていないものが退避された」である。**

⚠⚠ **利用者から見て何が起きるか:**
⚠ **URL は生きているように見え、⚠ 合言葉は正しく、⚠ それでも入れない。**
⚠ **そして拒否の応答は「合言葉が違う」と区別できない**
([`../.claude/rules/security.md`](../.claude/rules/security.md) § 3)。
⚠ **so 利用者には、⚠ 何が起きたのかを知る手立てが無い。**

#### ⚠ この測定が言えないこと

- ⚠ **`wrangler dev --local` の `workerd` で測った。** ⚠ **本番の Cloudflare ではない。**
  ⚠ **本番のほうが長く生きる可能性も、⚠ 短い可能性もある。**
- ⚠ **1 回ずつである。** ⚠ **境界を何度も確かめたわけではない。**
- ⚠ **so 「10 秒である」とは言わない。** ⚠ **言えるのは「8 秒は生き、⚠ 15 秒は死んだ」までである。**
...
- ⚠ **本番の Cloudflare で同じ時間で消えるか。** ⚠ **ローカルでしか踏んでいない。**
