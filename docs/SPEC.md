...
| ⚠⚠ **扉に立っている人** | ⚠ **待っている人が 読み込み直しても、⚠ Host の扉では 1 人のままである。⚠ 待機接続を失ったまま 30 秒たった人は 扉から降り、⚠ その人は 戻ったときに 新しいノックからやり直す。⚠ 相手が戻れば 両側の画面が そう言う** | — | ⚠ **ケース `one-person-one-knock` と `one-knock-once`**(`npm run e2e`)、⚠ **`test/knock.test.ts` の掃き出し、⚠ `test/heartbeat.test.ts` の「鼓動のたびに降ろす」、⚠ `test/signaling-session.test.ts` の `peer-here`**([`adr/0035`](adr/0035-let-the-door-hold-only-people-who-are-standing-there.md))。⚠⚠ **30 秒は 選んだ値であって 測った値ではない。** ⚠ **実機では まだ一度も走らせていない。** ⚠ **`pagehide` が鳴らずに端末が落ちた場合の挙動も 測っていない** |
...
| ⚠⚠ **「ここ」と指す** | ⚠ **共有画面の上でも、⚠ 相手のカメラ映像の上でも 指せる。⚠ 指した先は 相手の同じ絵の 同じ場所に出る。⚠ 指を離すと 消える** | W3C WebRTC(`RTCDataChannel`) | ⚠ **ケース `over-here`**(`npm run e2e`)。⚠ **点が「出た」ではなく、⚠ 割合で 同じ場所に落ちたことを見る**(⚠ **両側の要素の大きさは違う**)。⚠ **面を取り違えていないこと、⚠ 離すと消えることも 同じケースで見る**([`adr/0033`](adr/0033-share-a-screen-where-it-can-be-shared-and-point-over-a-data-channel.md))。⚠ **座標は kagima のサーバを通らない ― ⚠ `RTCDataChannel` である**(⚠ **`test/no-media-on-the-server.test.ts` はこれを見ていない。⚠ 見ているのは 型と import である**)。⚠ **Chromium のみ。** ⚠⚠ **実機 2026-09-11: ⚠ 出た** — ⚠ **ただし 共有エリア内でクリックしてからである**(⚠ Owner の報告)。⚠ **測った: ⚠ 指の tap は `pointermove` を 1 つも出さない** — ⚠ **so 滑らせない tap は 何も送っていなかった。** ⚠ **`pointerdown` でも送る。** ⚠⚠ **指を離しても 数秒は 残る**(⚠ Owner 決定 2026-09-11)— ⚠ **ケース `over-here` は、⚠ 「まだ出ている」を 1 回読むのではなく、⚠ **消えるまでの時間を測って** それが数秒であることを見る。** ⚠ **指しなおすと 仕掛けてあった消去が 取り消されることも 見る** |
| ⚠ **手を離す** | ⚠ **カメラを切ると track が止まる(⚠ ランプが消える)。⚠ mute は track を止めず、⚠ `enabled = false` にする。⚠ どちらも 相手の画面に そう出る** | W3C Media Capture(`MediaStreamTrack.stop` / `enabled`)、WebRTC(`RTCDataChannel`) | ⚠ **ケース `hands-off`**(`npm run e2e`)。⚠ **切る前に掴んだ track の `readyState` が `ended` になること、⚠ mute では `live` のまま `enabled` だけが落ちることを見る**(⚠ **2 つを取り違えると どちらかが必ず落ちる**)。⚠ **相手側では、⚠ 顔の要素が隠れ、⚠ 文が出て、⚠ 戻すと frames がまた増えることを見る**([`adr/0033`](adr/0033-share-a-screen-where-it-can-be-shared-and-point-over-a-data-channel.md))。⚠ **Chromium のみ。** ⚠⚠ **実サービス、⚠ 2026-09-10 の deploy 以降: ⚠ camera の on / off は「⚠ 機能していた」**(⚠ Owner の報告であって、⚠ 取得した記録ではない)。⚠⚠ **カメラを切ると 端末のランプが 消えた**(⚠ Owner の報告、⚠ 2026-09-11)— ⚠ **「隠すのではなく track を止めた」を実機で裏づけるのは、⚠ その一点だけである。** ⚠ **mute も「⚠ 機能している」**(⚠ 同じく報告であって、⚠ 取得した記録ではない) |
