...
| ⚠ **2026-09-06.** ⚠ **`docs-check`'s `ci-names-every-entry-point` searched the workflow for the string `npm run <tier>`.** ⚠ **Two things in the file satisfied it without running anything: ⚠ the step's own `- name:` label, ⚠ and — ⚠ worse — ⚠ the report line `echo "npm run e2e — did not reach it"`, ⚠ whose whole purpose is to say a tier did NOT run.** ⚠ **So the case could not fail, ⚠ for any tier, ⚠ ever.** ⚠ **Found by adding a fourth tier and mutating it away** | ⚠ **Match a command, ⚠ not a mention: ⚠ `^\s*npm run <tier>\b`.** ⚠ **A command starts its own line; ⚠ a label and an `echo` do not.** ⚠ **Proven by gutting each of the four tiers in turn and watching all four fail.** ⚠⚠ **A check that reads a file for a word will be satisfied by the word** — ⚠ **including by the sentence written to say the thing did not happen** |
| ⚠ **2026-09-06.** ⚠ **A wall named four files and read their own text.** ⚠ **`src/signaling/session.ts` claimed to be platform-free and passed** — ⚠ **while `./messages.ts`, ⚠ one import away, ⚠ used `Buffer`.** ⚠ **The four entry points reach ten files, ⚠ and the wall was looking at four** | ⚠ **A wall about what a file *does* must follow what it *loads*.** ⚠ **`test/reachable.ts` walks the relative imports; ⚠ the wall runs over everything reachable.** ⚠ **Proven by putting `Buffer` two hops away, ⚠ in `src/random.ts`, ⚠ and watching it get named.** ⚠ **The same wall now also asserts the dependency runs one way** — ⚠ **the core never loads the adapter, ⚠ or the split it describes has stopped existing.** ⚠ **Walls that sweep all of `src/` never had this gap; ⚠ ⚠ the gap belongs to a wall with a hand-written list** |
| ⚠ **2026-09-12.** ⚠ **The Host's page kept a list of who is at the door, ⚠ and appended to it whenever the server announced a knock.** ⚠ **The server announces every waiting knock again on each Host connection** (`src/signaling/session.ts`) — ⚠ **on purpose, ⚠ so a knock that arrived while the socket was away is not lost.** ⚠⚠ **So one reconnect made one person into two, ⚠ and "入れる" removed only one: ⚠ the door went on saying somebody was knocking after that somebody had already come in.** ⚠ **Found on a real device by the owner, ⚠ not by any check** | ⚠ **A list of people is keyed by who they are, ⚠ never by how many times it was told about them.** ⚠ **The page holds each `knockId` once.** ⚠ **The wall is `npm run e2e --only=one-knock-once`**: ⚠ **it cuts the sockets with somebody at the door, ⚠ waits for the reconnect, ⚠ decides once, ⚠ and requires the door to close AND that person to actually get in.** ⚠⚠ **Writing that wall found a second one: ⚠ pressing "入れる" while the socket was down removed the person from the door although the message never left the page** — ⚠ **so the Host saw nobody and the Guest waited for ever.** ⚠ **Now the list only lets go once the decision can actually be sent** |
| ⚠ **2026-09-08.** ⚠ **A `Response` was built at module scope, ⚠ and the Worker would not start at all** — ⚠ **`Disallowed operation called within global scope`.** ⚠ **The version it replaced held a `Response` there too and was fine, ⚠ so the shape looked proven** — ⚠ **the difference was that the old one had `null` for a body and the new one had a string.** ⚠ **`src/token/join-token.ts` already carried the same lesson about drawing random values, ⚠ and it was read and not applied** | ⚠ **Build nothing at module scope that a runtime could call I/O.** ⚠ **A `Response` with a body counts.** ⚠ **The wall is `npm run worker`** — ⚠ **it does not assert this, ⚠ it simply cannot start, ⚠ and that is enough.** ⚠ **Watched it catch this one before the fix and pass after.** ⚠⚠ **"The same shape worked before" is not a check** — ⚠ **the shape that worked differed in the one way that mattered** |
